...
export default function GraficoEstadisticasSet({ setId, token }) {
  ...
  const equiposData = useMemo(() => {
    ...
    estadisticas.forEach(stat => {
      ...
      const equipoId = (equipoObj && (equipoObj._id || equipoObj)) || 'sin-equipo';
      const equipoNombre = (equipoObj && equipoObj.nombre) || 'Equipo';
  ...
  const datosGrafico = useMemo(() => {
    ...
    estadisticas.forEach(stat => {
      ...
      if (!equiposUnicos[equipoId]) {
        equiposUnicos[equipoId] = {
          nombre: (equipoObj && equipoObj.nombre) || 'Equipo',
    ...
    estadisticas.forEach((stat, index) => {
      ...
      datos.push({
        ...
        equipo: (equipoObj && equipoObj.nombre) || 'Equipo',
        equipoId: equipoId,
        equipoIndex: equiposUnicos[equipoId]?.index ?? 0,
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {equipoInfo.jugadores.map((stat, index) => {
                      ...
                      if (stat.jugador) {
                        ...
                        } else if (typeof stat.jugador.nombre === 'string' && stat.jugador.nombre.trim()) {
                          jugadorNombre = stat.jugador.nombre;
                        }
                      }
