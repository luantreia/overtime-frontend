...
export default function ModalEstadisticasCaptura({
  ...
  // Override de UI para limpiar sin tocar backend
  const [uiStatsOverride, setUiStatsOverride] = useState(null);
    ...
    <ModalLayout onClose={onClose}>
      ...
      <div className="space-y-4 px-1 pb-4">
        ...
        {estadisticasSet && (
          <>
            {(() => {
              // Normalizar equipos: soportar string (ID) u objeto populate
              const eqLocalObj = typeof partidoLocal.equipoLocal === 'string'
                ? { _id: partidoLocal.equipoLocal, nombre: 'Equipo Local' }
                : (partidoLocal.equipoLocal || { _id: 'sin-equipo', nombre: 'Equipo Local' });
              const eqVisitObj = typeof partidoLocal.equipoVisitante === 'string'
                ? { _id: partidoLocal.equipoVisitante, nombre: 'Equipo Visitante' }
                : (partidoLocal.equipoVisitante || { _id: 'sin-equipo', nombre: 'Equipo Visitante' });

              const localStats = uiStatsOverride ? uiStatsOverride.local : mapEquipo(eqLocalObj._id);
              const visitStats = uiStatsOverride ? uiStatsOverride.visitante : mapEquipo(eqVisitObj._id);

              return (
                <EquiposEstadisticas
                  equipoLocal={eqLocalObj}
                  equipoVisitante={eqVisitObj}
                  estadisticas={{ local: localStats, visitante: visitStats }}
                  onCambiarEstadistica={cambiarEstadistica}
                  onAsignarJugador={(equipo, index, jugadorId) => {
                    const equipoId = equipo === 'local' ? eqLocalObj._id : eqVisitObj._id;
