...
export default function SeccionContratosJugadorEquipo({
  ...
  const [lista, setLista] = useState(Array.isArray(jugadoresEquipo) ? jugadoresEquipo : []);
  ...
  const cargarSolicitudesEdicion = async () => {
    try {
      ...
      console.warn('Error cargando solicitudes de edición (equipo):', e?.message || e);
    }
  };
  ...
  const guardarContratoEditado = async (contratoId) => {
    try {
      ...
      console.error('Error guardando contrato (equipo):', err);
    }
  };
  ...
  useEffect(() => {
    setLista(Array.isArray(jugadoresEquipo) ? jugadoresEquipo : []);
    cargarSolicitudesEdicion();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(jugadoresEquipo)]);
              ...
              {lista.map((c) => (
                <React.Fragment key={c._id}>
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">{c.desde ? new Date(c.desde).toLocaleDateString() : '-'}</td>
                    <td className="px-4 py-2">{c.hasta ? new Date(c.hasta).toLocaleDateString() : '-'}</td>
