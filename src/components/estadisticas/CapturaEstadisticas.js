  ...
  const renderJugadorSlot = (equipo, posicion, equipoId, equipoNombre, colorClass, bgClass) => {
    ...
    const jugador = jugadorSeleccionado ?
      (equipo === 'local' ? jugadoresLocal : jugadoresVisitante)
        .find(j => (j.jugadorPartidoId || j._id) === jugadorSeleccionado) : null;

    const stats = estadisticas[jugadorSeleccionado] || {};

    return (
      <div key={`${equipo}-${posicion}`} className={`${bgClass} border rounded-lg p-3`}>
        ...
        {jugador && (
          <div className="space-y-1">
            <div className="text-xs text-gray-600">{jugador.nombre || 'Jugador'}</div>
            <div className="grid grid-cols-4 gap-1 text-xs">
              <div>
                <label className="block text-gray-500">Throws</label>
                <input
                  type="number"
                  min="0"
              ...
              <div>
                <label className="block text-gray-500">Hits</label>
                <input
                  type="number"
                  min="0"
                  value={stats.hits || 0}
              ...
              <div>
                <label className="block text-gray-500">Outs</label>
                <input
                  type="number"
                  min="0"
                  value={stats.outs || 0}
              ...
              <div>
                <label className="block text-gray-500">Catches</label>
                <input
                  type="number"
                  min="0"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {Array.from({ length: 10 }, (_, i) =>
              renderJugadorSlot('local', i, partido?.equipoLocal?._id, 'Local', 'text-blue-800', 'bg-blue-50')
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {Array.from({ length: 10 }, (_, i) =>
              renderJugadorSlot('visitante', i, partido?.equipoVisitante?._id, 'Visitante', 'text-red-800', 'bg-red-50')
            )}
          </div>
        </div>
      </div>
